...
# Follow the interactive menu to set up your chosen backend
sudo paqctl menu
```
...
```powershell
cd C:\path\to\paqctl
# Example: cd C:\Users\YourName\Downloads\paqctl
```
...
- اگر شبکه شما سانسور سنگین دارد (مثل GFW چین): **ابتدا GFK را امتحان کنید**
...
```powershell
cd C:\path\to\paqctl
# مثال: cd C:\Users\YourName\Downloads\paqctl
```
...
2. اسکریپت:
   - **Npcap** را دانلود و نصب می‌کند
   - **باینری paqet** را دانلود می‌کند
...
2. اسکریپت:
   - **Npcap** را نصب می‌کند
   - **Python 3.10+** را نصب می‌کند
   - پکیج‌های Python را نصب می‌کند
